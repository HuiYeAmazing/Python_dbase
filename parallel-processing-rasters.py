...
var_str = 'NPP'
raster_type = '.flt'
f_coef = dest_folder + os.sep + var_str + '_coef.tif'
f_r2score = dest_folder + os.sep + var_str + '_r2score.tif'
f_pvalue = dest_folder + os.sep + var_str + '_pvalue.tif'
...
def linear_trend (data_x, data_y):
    """
    :param data_x:  时间轴 or independ variable
    :param data_y: 需要计算趋势和显著性水平的数组 P * N array
    :return: 返回趋势r2score和显著性水平pvalue
    """
    
    df = pd.DataFrame({"indepent": data_x, "depent": data_y})   # 建立dataframe
...
if __name__ == '__main__':
 # 将数组转换成以象元数为行数，年份为列数的数组
    ...
    pool = Pool(cores)
    Regres_ = pool.map(linear_trend, data_x, data_y)
